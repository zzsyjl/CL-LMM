...
from llava.train.sparsegpt import SparseGPT
...
class ModelArguments:
    model_name_or_path: Optional[str] = field(default="path of finetuned model")
    model_base: Optional[str] = field(default=None)
    pre_model_name_or_path: Optional[str] = field(default="path of pretrained model(llava_v1_5_7b)")
    pre_model_base: Optional[str] = field(default=None)
    version: Optional[str] = field(default="v1")
    freeze_backbone: bool = field(default=True)
    tune_mm_mlp_adapter: bool = field(default=False)
    vision_tower: Optional[str] = field(default="path of clip_vit_large_patch14_336")
    mm_vision_select_layer: Optional[int] = field(default=-2)   # default to the last layer
    pretrain_mm_mlp_adapter: Optional[str] = field(default="path of mm_projector.bin")
...
@dataclass
class DataArguments:
    data_path: str = field(default="path of target data file",
...
def train():
    ...
    pre_tokenizer, pre_model, pre_image_processor, pre_context_len = load_pretrained_model(pre_model_path, model_args.pre_model_base, pre_model_name)
    ...
    # layers.append(model.model.mm_projector)
    dtype = next(iter(model.model.parameters())).dtype
    inps = {}
    pretrain_layers = pre_model.model.layers
    # pretrain_layers.append(pre_model.model.mm_projector)
    ...
    for i in range(20, len(layers)):
        # layer = layers[i].to(training_args.device)
        # pretrain_layer = pretrain_layers[i].to(training_args.device)
        layer = layers[i]
        pretrain_layer = pretrain_layers[i]

        subset = find_layers(layer)#每个LLM layer下的所有卷积层和线性层
